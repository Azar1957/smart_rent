    ...
    <div className="bg-canvas">
      <div className="mx-auto max-w-page px-6 py-10">
        ...
        <h1 className="text-heading-sm md:text-heading font-bold text-obsidian tracking-heading">
          {t.payments.rentTitle}
        </h1>
        <p className="text-body-sm text-mistx mt-2 max-w-xl">
          {user.role === 'tenant' ? t.payments.rentSubTenant : t.payments.rentSubLandlord}
        </p>
        ...
        <nav className="mt-6 flex items-center gap-3 text-body-sm">
          ...
          <Link href="/payments/utilities" className="nav-link">
            {t.payments.utilTabTitle}
          </Link>
        </nav>

        <div className="mt-8">
          <PaymentsTable rentals={rentals} filterKind="rent" role={user.role} />
        </div>
      </div>
    </div>
  );
